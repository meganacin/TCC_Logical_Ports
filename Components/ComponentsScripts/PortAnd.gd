...
func _process(delta):
	...
	portValue1 = $PortAnd1.portValue
	portValue2 = $PortAnd2.portValue
	...
	if objectSelected:
		seguirMouse()
		$AND.visible=true
		get_node("PortAnd1/CollisionShape2D").disabled = true
		get_node("PortAnd2/CollisionShape2D").disabled = true
	...
	if (onDeleteArea and objectSelected==false):
		...
		get_node("PortAnd1/CollisionShape2D").disabled = false
		get_node("PortAnd2/CollisionShape2D").disabled = false
...
func _on_PortAnd_body_entered(body):
	if(body.name == "Lixeira"):
		onDeleteArea = true
		body.openTrash("AND")


func _on_PortAnd_body_exited(body):
	if(body.name == "Lixeira"):
		onDeleteArea = false
		body.closeTrash("AND")
